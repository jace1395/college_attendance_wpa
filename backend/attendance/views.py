...
from reportlab.platypus import PageBreak, BaseDocTemplate, PageTemplate, Frame, Paragraph, Spacer, Table, TableStyle
        ...
        data = [
            {
                ...
                "action": log.action,
                "detail": log.detail,
                "time": timezone.localtime(log.created_at).strftime('%Y-%m-%d %I:%M %p'),
                "color": log.color
            }
...
class TeacherMonitoringDutyView(APIView):
    ...
    def get(self, request):
        ...
        duties = MonitoringDuty.objects.filter(teacher=user).exclude(class_batch__academic_year='Archived').select_related('class_batch__subject').order_by('-date', 'time_slot')
...
class AdminPrincipalManagementView(APIView):
    ...
    def post(self, request):
        ...
        if action == 'update':
            ...
            if email:
                ...
                if User.objects.filter(email=email).exclude(id=principal.id).exists():
                    return Response({"error": "Email already in use"}, status=400)
                principal.email = email
            principal.save()
            return Response({"success": "Principal updated successfully"})
...
class AdminDashboardView(APIView):
    ...
    def get(self, request):
        ...
        active_students = User.objects.filter(role__iexact='Student', last_activity__gte=time_threshold).count()
        active_teachers = User.objects.filter(Q(role__iexact='Teacher') | Q(role__iexact='HOD'), last_activity__gte=time_threshold).count()
...
class AdminBackupExportView(APIView):
    ...
    def get(self, request):
        ...
        qs = Attendance.objects.select_related(
            'student', 'student__stream', 'student__department', 'class_batch__subject', 'class_batch__teacher'
        )
        if academic_year != 'All':
            qs = qs.filter(class_batch__academic_year=academic_year)
            
        # For all exports (Excel, PDF, CSV), we want the full history.
        qs = qs.order_by('-date')
        ...
        for attendance in qs:
            stream_name = attendance.student.stream.name if attendance.student.stream else "Unassigned"
            subject_name = attendance.class_batch.subject.name if attendance.class_batch and attendance.class_batch.subject else "Unknown Subject"
            grouped_records[stream_name][subject_name].append(attendance)
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
            if export_type == 'pdf':
                ...
                # Use landscape A4 and smaller margins for wide tables
                doc = BaseDocTemplate(file_buffer, pagesize=landscape(A4),
                ...
                elements.append(Paragraph(f"College Backup ({academic_year})", title_style))
                ...
                for stream_name, subjects in sorted(grouped_records.items()):
                    elements.append(Paragraph(f"<b>{stream_name}</b>", styles['Heading3']))
                    for subject_name, records in sorted(subjects.items()):
                        # Try to find teacher
                        teacher_name = "Unknown Teacher"
                        for r in records:
                            if r.class_batch and r.class_batch.teacher:
                                teacher_name = r.class_batch.teacher.name or r.class_batch.teacher.email
                                break
                        
                        anchor_name = f"{stream_name}_{subject_name}".replace(' ', '_').replace('/', '_')
                        link = f'<a href="#{anchor_name}" color="blue">{subject_name} ({teacher_name})</a>'
                        elements.append(Paragraph(f"&nbsp;&nbsp;&nbsp;&nbsp;&bull; {link}", styles['Normal']))
                
                elements.append(PageBreak())
                
                # Build Pages for each subject
                for stream_name, subjects in sorted(grouped_records.items()):
                    for subject_name, records in sorted(subjects.items()):
                        teacher_name = "Unknown Teacher"
                        for r in records:
                        ...
                        anchor_name = f"{stream_name}_{subject_name}".replace(' ', '_').replace('/', '_')
                        elements.append(Paragraph(f'<a name="{anchor_name}"/><b>{stream_name} - {subject_name}</b> (Teacher: {teacher_name})', styles['Heading2']))
                        elements.append(Spacer(1, 12))
                        ...
                        for d in unique_dates:
                            dynamic_headers.append(d.strftime('%d-%b'))
                        for yr, wk in unique_weeks:
                            dynamic_headers.append(f"Wk {wk}")
                        for yr, mo in unique_months:
                            dynamic_headers.append(f"{datetime.date(yr, mo, 1).strftime('%b')}")
                        dynamic_headers.extend(["Tot", "Att", "Overall%"])
                        ...
                        student_data = []
                        sorted_students = sorted(student_records.items(), key=lambda x: natural_keys(x[0].roll_no or x[0].id))
                        for student, s_records in sorted_students:
                            base_row = [student.roll_no or str(student.id), student.name or student.email]
                            dyn_row = []
                            dyn_styles = [] # (index in dyn_row, color)
                            ...
                            for d in unique_dates:
                                ...
                                if status == "Present":
                                    dyn_row.append("P")
                                    dyn_styles.append((len(dyn_row)-1, colors.green))
                                elif status == "Absent":
                                    dyn_row.append("A")
                                    dyn_styles.append((len(dyn_row)-1, colors.red))
                                else:
                                    dyn_row.append("-")
                            ...
                            for yr, wk in unique_weeks:
                                ...
                                dyn_row.append(f"{pct}%")
                                dyn_styles.append((len(dyn_row)-1, colors.red if pct < 75 else colors.green))
                            ...
                            dyn_row.extend([str(total), str(attended), f"{pct}%"])
                            dyn_styles.append((len(dyn_row)-1, colors.red if pct < 75 else colors.green))
                            
                            student_data.append((base_row, dyn_row, dyn_styles))
                            
                        # Chunking columns only, let ReportLab handle row pagination
                        COL_CHUNK_SIZE = 17
                        
                        for c_idx in range(0, len(dynamic_headers), COL_CHUNK_SIZE):
                            chunk_headers = ["Roll No", "Name"] + dynamic_headers[c_idx:c_idx+COL_CHUNK_SIZE]
                            ...
                            for base_row, dyn_row, dyn_styles in student_data:
                                row_idx = len(table_data)
                                row = list(base_row) + dyn_row[c_idx:c_idx+COL_CHUNK_SIZE]
                                table_data.append(row)
                                
                                for dyn_idx, c in dyn_styles:
                                    if c_idx <= dyn_idx < c_idx + COL_CHUNK_SIZE:
                                        col_idx = 2 + (dyn_idx - c_idx)
                                        table_styles.append(('BACKGROUND', (col_idx, row_idx), (col_idx, row_idx), c))
                                        
                            t = Table(table_data, repeatRows=1)
                            t.setStyle(TableStyle(table_styles))
                            elements.append(t)
                            # Put each column chunk on a new page to keep tables readable
                            elements.append(PageBreak())
                        
                doc.build(elements)
                zf.writestr(f"College_Backup_{academic_year}.pdf", file_buffer.getvalue())
                ...
                # Generate unique tab colors for each stream
                stream_colors = {}
                color_palette = ['FF9999', '99FF99', '9999FF', 'FFFF99', 'FF99FF', '99FFFF', 'FFCC99', 'CC99FF']
                color_idx = 0
                
                for stream_name, subjects in grouped_records.items():
                    if stream_name not in stream_colors:
                        stream_colors[stream_name] = color_palette[color_idx % len(color_palette)]
                        color_idx += 1
                        
                    for subject_name, records in subjects.items():
                        base_title = re.sub(r'[\\*?:/\[\]]', '_', f"{stream_name[:10]}_{subject_name[:15]}")
                        ...
                        ws.sheet_properties.tabColor = stream_colors[stream_name]
                        ...
                        sorted_students = sorted(student_records.items(), key=lambda x: str(x[0].roll_no or x[0].id))
                        for student, s_records in sorted_students:
                            ...
                            for col_idx, val in enumerate(row, start=1):
                                ...
                                elif isinstance(val, (int, float)) and col_idx not in (total_col, attended_col):
                                    if val < 75:
                                        ws.cell(row=current_row, column=col_idx).fill = red_fill
                                    else:
                                        ws.cell(row=current_row, column=col_idx).fill = green_fill
                ...
                file_buffer = io.BytesIO()
                wb.save(file_buffer)
                zf.writestr(f"College_Backup_{academic_year}.xlsx", file_buffer.getvalue())
                ...
                for stream_name, subjects in grouped_records.items():
                    for subject_name, records in subjects.items():
                        ...
                        zf.writestr(f"{safe_stream}/{safe_subject}.csv", text_buffer.getvalue().encode('utf-8'))
...
class PrincipalFilterOptionsView(APIView):
...
class PrincipalAdvancedGraphView(APIView):
    permission_classes = [IsAuthenticated, IsPrincipal]

    def get(self, request):
        streams = request.query_params.getlist('streams[]')
        years = request.query_params.getlist('years[]')
        classes = request.query_params.getlist('classes[]')
        
        # Build base queryset
        qs = Attendance.objects.all()
        
        if streams:
            qs = qs.filter(class_batch__subject__stream__in=streams)
        ...
        if classes:
            # classes filter (class_batch.id)
            qs = qs.filter(class_batch__id__in=classes)
...
class PrincipalMonitoringDutiesView(APIView):
    permission_classes = [IsAuthenticated, IsPrincipal]

    def get(self, request):
        duties = MonitoringDuty.objects.select_related('teacher', 'class_batch', 'class_batch__subject').order_by('-date', '-time_slot')
        data = []
        for d in duties:
            enrolled = 0
...
class PrincipalDashboardView(APIView):
    ...
    def get(self, request):
        ...
        # Stream-wise statistics for graphs
        from django.db.models import Count, Q
        
        stream_data = Attendance.objects.values('class_batch__subject__stream').annotate(
            present=Count('id', filter=Q(status='Present')),
            total=Count('id')
        )
        stream_stats = []
        for s in stream_data:
            stream_name = s['class_batch__subject__stream'] or "Unassigned"
        ...
        # Class-wise statistics for graphs
        class_data = Attendance.objects.values('class_batch__subject__semester', 'class_batch__subject__stream').annotate(
            present=Count('id', filter=Q(status='Present')),
            total=Count('id')
        )
        grouped_classes = {}
        for c in class_data:
            ...
            grouped_classes[key]["present"] += c["present"]
            grouped_classes[key]["total"] += c["total"]
            
        class_stats = []
        for key, vals in grouped_classes.items():
...
class AdminFilterOptionsView(APIView):
    ...
    def get(self, request):
        ...
        streams = Stream.objects.all().values('id', 'name', 'department_id')
        
        # We need classes with their subject and teacher info
...
class AdminStudentHierarchyView(APIView):
    permission_classes = [IsAuthenticated, IsAdmin]
...
class AdminTeacherHierarchyView(APIView):
    permission_classes = [IsAuthenticated, IsAdmin]
    ...
    def get(self, request):
        ...
        for t in qs:
            classes_assigned = []
            for cb in t.classbatch_set.all():
                classes_assigned.append({
                    'id': cb.id,
                    'subject_name': cb.subject.name,
                    'stream': cb.subject.stream,
                    'semester': cb.subject.semester,
        ...
        return Response({'teachers': teachers_data})

class AdminHODHierarchyView(APIView):
    permission_classes = [IsAuthenticated, IsAdmin]
...
class AdminMentorHierarchyView(APIView):
    ...
    def get(self, request):
        ...
        for m in qs:
            ...
            mentees_data = sorted(mentees_data, key=lambda x: str(x['roll_no'] or x['id']))
