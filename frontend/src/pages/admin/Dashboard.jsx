...
import SystemOverrides from '../../components/admin/SystemOverrides';
        ...
        setTimeout(() => {
          setDashboardData({
            ...
            system_stats: {
              ...
              total_teachers: 142,
              active_sessions: 312,
              last_database_backup: "2026-08-23T02:00:00Z"
            },
            recent_audit_logs: [
              { action: "User Created", target: "2511011.jace.sdcce@vvm.edu.in", timestamp: "10:15 AM - Today" },
              { action: "System Backup", target: "DB_Auto_Routine", timestamp: "02:00 AM - Today" },
              { action: "Role Modified", target: "sumit.kumar@vvm.edu.in -> HOD", timestamp: "04:30 PM - Yesterday" },
              { action: "Master Unlock", target: "Class BVC_SEM3 by Principal", timestamp: "11:20 AM - Yesterday" },
              { action: "Bulk Upload", target: "45 Students Added via CSV", timestamp: "09:00 AM - Yesterday" }
            ]
          });
          setLoading(false);
        }, 600);
      ...
      <div className="relative z-10 p-4 md:p-8 max-w-7xl mx-auto min-h-screen flex flex-col">
        ...
        <div className="flex flex-wrap justify-between items-center mb-8 gap-4 bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-4 md:p-6 shadow-2xl">
          <div className="flex items-center gap-4">
              ...
              <div>
                  ...
                  <p className="text-sm text-white/50">{admin?.email} • <span className="text-blue-300">{admin?.role}</span></p>
              </div>
          </div>
        ...
        <div className="flex justify-start mb-8 overflow-x-auto pb-2">
            <div className="flex gap-2 bg-slate-900/50 p-1.5 rounded-2xl border border-white/10 shrink-0">
                {['overview', 'users', 'overrides', 'reports'].map(tab => (
                    <button
                        key={tab}
