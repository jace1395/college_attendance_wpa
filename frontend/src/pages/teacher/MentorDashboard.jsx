...
const MentorDashboard = () => {
  ...
  const streams = ["BCA"]; // Mock mentor scope
      ...
      <div className="relative z-10 p-4 md:p-8 max-w-7xl mx-auto min-h-screen flex flex-col">
        ...
        <div className="flex flex-wrap justify-between items-center mb-8 gap-4">
          <h1 className="text-2xl font-bold tracking-wider">
            VVM <span className="text-blue-500">MENTOR PORTAL</span>
          </h1>
        ...
        <div className="flex flex-wrap justify-between items-center mb-6 gap-4 border-b dark:border-white/10 border-slate-300 pb-4">
          <div>
            <h2 className="text-2xl font-bold dark:text-white/90 text-slate-800">Welcome, {user?.name || "Mentor"}</h2>
            <p className="text-blue-600 dark:text-blue-300 font-medium">Assigned Mentees Overview</p>
          </div>
              ...
              {['dashboard', 'reports'].map(tab => (
                  <button
                      ...
                      className={`px-6 py-2.5 rounded-xl text-sm font-bold capitalize transition-all ${
                          activeTab === tab ? 'bg-blue-600 text-white shadow-lg' : 'dark:text-white/60 text-slate-600 hover:text-blue-600 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5'
                      }`}
                  >
                      {tab}
                  </button>
        ...
        <div className="mt-4 flex-1">
          {activeTab === 'dashboard' && (
            <div className="dark:bg-white/5 bg-white backdrop-blur-xl border border-blue-500/30 rounded-3xl p-6 text-center py-10 shadow-xl animate-fade-in-up">
               <p className="text-xl dark:text-white/60 text-slate-500 mb-4">Welcome to the standalone Mentor Dashboard.</p>
               <button onClick={() => setActiveTab('reports')} className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 px-6 rounded-xl transition-all">View Mentee Reports</button>
            </div>
          )}
          {activeTab === 'reports' && <PrincipalReportsHub streams={streams} onNavigateToView={() => setActiveTab('reports')} />}
        </div>
      </div>
    </div>
  );
