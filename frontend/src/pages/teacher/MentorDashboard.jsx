...
const MentorDashboard = ({ onBack }) => {
  ...
  if (selectedMentee) {
    return <StudentAttendanceModal studentId={selectedMentee.id} onBack={() => setSelectedMentee(null)} />;
  }
    ...
    <div className="flex flex-col gap-8 animate-fade-in">
      ...
      <div className="bg-emerald-900/10 border border-emerald-500/20 rounded-3xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">Mentor Panel</h2>
      ...
      <div className="bg-slate-200/50 dark:bg-white/5 border border-slate-300/60 dark:border-white/10 rounded-2xl overflow-hidden shadow-xl">
        <div className="px-5 py-4 border-b border-slate-300/60 dark:border-white/10 flex flex-wrap items-center gap-3">
          <h3 className="text-sm font-bold text-slate-700 dark:text-white/80">My Mentees</h3>
          <div className="ml-auto relative">
            <svg className="w-3.5 h-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-white/30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <input
              ...
              className="bg-slate-900/60 text-slate-700 dark:text-white/80 rounded-xl pl-9 pr-4 py-2 text-xs border border-slate-300/60 dark:border-white/10 focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none w-48"
            />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-14 text-center text-slate-400 dark:text-white/30 flex flex-col items-center gap-3">
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {filteredMentees.map(m => (
                  <tr key={m.roll} className={`border-t border-slate-300/50 dark:border-white/5 hover:bg-slate-200/50 dark:bg-white/5 transition-colors ${m.attendance_pct < 75 ? 'bg-red-900/10' : ''}`}>
                    ...
                    <td className="px-5 py-3">
                      <div className="w-full bg-slate-800/60 rounded-full h-2 overflow-hidden">
                        <div
                          className={`h-full rounded-full bg-linear-to-r ${getBarColor(m.attendance_pct)}  `}
